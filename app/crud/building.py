...
def update_building(db: Session, building_id: int, building: BuildingCreate) -> BuildingModel:
    db_building = db.query(BuildingModel).filter(BuildingModel.id == building_id).first()
    if not db_building:
        return None
