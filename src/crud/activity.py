...
def create_activity(db: Session, activity: ActivityCreate) -> ActivityModel:
    if activity.parent_id:
        parent = db.query(ActivityModel).filter(
            ActivityModel.id == activity.parent_id).first()
        if not parent:
            raise HTTPException(status_code=404,
                                detail="Parent activity not found")
        if parent.level >= 3:
            raise HTTPException(status_code=400,
                                detail="Maximum nesting level reached")
